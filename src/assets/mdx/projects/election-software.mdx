...
Supported 20+ concurrent voters with total voter count reaching 1000+ votes, with **no discrepency** with the manual head count.
...
Frontend Built on vanilla typescript stack using ExpressJS as backend and was Used for setting up posts and candidates


### Election Runner

Frontend built on vanilla typescript and used a FastAPI based backend.
Stored vote-session objects in MongoDB which were later analysed and converted into a result report.
...
Instant Result display powered by matplotlib
